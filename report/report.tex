...
\tikzstyle{startstop} = [rectangle, rounded corners, minimum width=0cm, minimum height=0cm, text centered, text width=2cm, draw=black, fill=white!30]
\tikzstyle{process} = [rectangle, minimum width=0cm, minimum height=0cm, text centered, text width=5.2cm, draw=black, fill=white!30]
...
\begin{figure}[H]
  \centering
  \subfloat[\texttt{NoEntry5.jpg}]{\includegraphics[width = 0.24\textwidth]{images/task_3_5.jpg}}
  \hfill
  \subfloat[\texttt{NoEntry5.jpg}]{\includegraphics[width = 0.24\textwidth]{images/task_3_15.jpg}}
  \hfill
  \subfloat[\texttt{NoEntry5.jpg}]{\includegraphics[width = 0.24\textwidth]{images/15_hough_space.jpg}}
  \hfill
  \subfloat[\texttt{NoEntry5.jpg}]{\includegraphics[width = 0.24\textwidth]{images/5_gradient_magnitude_threshold.jpg}}
  \hfill
  \subfloat[\texttt{NoEntry5.jpg}]{\includegraphics[width = 0.24\textwidth]{images/5_hough_space.jpg}}
  \hfill
  \subfloat[\texttt{NoEntry5.jpg}]{\includegraphics[width = 0.24\textwidth]{images/15_gradient_magnitude_threshold.jpg}}
...
\begin{table}[htbp]
  ...
  \begin{tabular}{l | l l | l l} 
    ...
    \texttt{NoEntry1.jpg}&1.00&1.00&+0.00&+0.87\\
    \texttt{NoEntry2.jpg}&1.00&1.00&+0.00&+0.92\\
    \texttt{NoEntry3.jpg}&1.00&0.80&+0.50&+0.65\\
    \texttt{NoEntry4.jpg}&1.00&1.00&+0.00&+0.84\\
    \texttt{NoEntry5.jpg}&0.10&0.15&$-0.20$&$-0.02$\\
    \texttt{NoEntry6.jpg}&0.50&0.67&+0.50&+0.67\\
    \texttt{NoEntry7.jpg}&0.00&0.00&+0.00&+0.00\\
    \texttt{NoEntry8.jpg}&0.43&0.60&$-0.14$&$-0.02$\\
    \texttt{NoEntry9.jpg}&1.00&1.00&+1.00&+1.00\\
    \texttt{NoEntry10.jpg}&0.67&0.80&+0.00&+0.47\\
    \texttt{NoEntry11.jpg}&0.50&0.67&+0.00&+0.56\\
    \texttt{NoEntry12.jpg}&0.13&0.22&$-0.25$&+0.01\\
    \texttt{NoEntry13.jpg}&0.00&0.00&+0.00&+0.00\\
    \texttt{NoEntry14.jpg}&0.00&0.00&+0.00&+0.00\\
...
\begin{tikzpicture}[node distance = 1.5cm]
  ...
  \node (hough) [process, below of = start] {Create set of bounding boxes of circles detetced with circle Hough transform};
  \node (vj) [process, below of = hough, yshift = -1em] {Create set of no entry sign bounding boxes using Viola-Jones detector};
  \node (cartesian) [process, below of = vj] {Create cartesian product of bounding boxes from both sets};
  \node (for) [process, below of = cartesian] {For each combination of bounding boxes in the cartesian product};
  \node (iou) [decision, below of = for, yshift = -5em] {IoU of\\combination greater\\than 50\%?};
  \node (list) [process, below of = iou, yshift = -3em] {Add circle bounding box to list of positive no entry signs};
  \node (done) [decision, right of = iou, xshift = 10em] {Has the IoU\\of all combinations\\been\\calculated?};
  \node (stop) [startstop, below of = done, yshift = -2em] {Stop};
  ...
  \draw let \p1 = (done.north) in [thick,-] (done.north) -- node[anchor = west] {No} (\x1, -7.5);
  \draw let \p1 = (done.north), \p2 = (iou.north) in [arrow] (\x1, -7.5) -- node[anchor = north] {Next combination} (\x2, -7.5);
